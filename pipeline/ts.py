...
class RecruitmentETL:
    ...
    def _init_industry_map(self):
        return {
            ...
            "software architecture", "kiến trúc phần mềm", "solution architect",
            "software tester", "qa engineer", "qa/qc", "tester", "qc", "qa",
            "software project manager", "project manager", "pm", "quản trị dự án",
            "software business analysis", "business analyst", "ba",
            "devops", "sysadmin", "system admin", "cloud", "aws", "azure"
        ],
        "An toàn thông tin": [
            "security engineer", "an toàn thông tin", "bảo mật", "security", "cyber security", "pentest"
        ],
        "IOT / Nhúng": [
            "iot", "internet of things", "nhúng", "embedded", "firmware", "vi mạch"
        ],
        "Help Desk / Support": [
            "it helpdesk", "help desk", "hỗ trợ kỹ thuật", "support", "kỹ thuật viên"
        ],
        "Dữ liệu & AI": [
            "data engineer", "kỹ sư dữ liệu",
            "data analyst", "bi", "business intelligence", "phân tích dữ liệu",
            "ai", "artificial intelligence", "trí tuệ nhân tạo", "machine learning", "deep learning", 
            "data scientist", "big data"
        ],
        "Game": [
            "game developer", "lập trình game", "unity", "cocos", "game designer", "unreal"
        ],
        # --- NHÓM MẶC ĐỊNH ---
        "IT - Khác": [] 
...
#Xử lý lương
    def clean_salary(self, raw_salary):
        if not isinstance(raw_salary, str) or not raw_salary: return 0.0, 0.0
        text = raw_salary.lower().replace(',', '')
        unit = 1
        if "usd" in text or "$" in text: unit = 25
        elif any(x in text for x in ["triệu", "tr", "m"]): unit = 1
        matches = re.findall(r'\d+(?:\.\d+)?', text)
        nums = [float(n) for n in matches]
        if not nums: return 0.0, 0.0
        if len(nums) == 1:
            val = nums[0] * unit
            if any(k in text for k in ["đến", "tới", "up to", "max"]): return 0.0, val
            if any(k in text for k in ["từ", "min", "from"]): return val, 0.0
            return val, val
...
#Xử lý cấp bậc
    def _extract_rank_strict(self, text):
        if pd.isna(text) or text == '': return "Junior"
        t = str(text).lower()
        if any(x in t for x in ['thực tập', 'intern']): return "Internship"
        if any(x in t for x in ['project manager', ' pm']): return "Project Manager"
        if any(x in t for x in ['leader', 'team lead', 'trưởng nhóm', 'manager']): return "Leader"
        if any(x in t for x in ['senior', 'sr.', 'cao cấp']): return "Senior"
        if any(x in t for x in ['mid-level', 'middle']): return "Mid-level"
        if any(x in t for x in ['fresher', 'mới tốt nghiệp']): return "Fresher"
        return "Junior"
#Xử lý loại hình lam việc
    def _determine_employment_type(self, text):
        ...
        if any(k in t for k in ['part time', 'bán thời gian']): return "Part-time"
        return "Full-time"
#Xử lý kiểu làm việc
...
#xử lý học vấn
    ...
    def find_education_coalesced(self, row):
        res = self.clean_education(str(row.get('HocVan', '')))
        if res == "Không yêu cầu":
            res = self.clean_education(str(row.get('YeuCauUngVien', '')))
        return res
#Xử lý kĩ năng
...
#Xử lý vị trí
    ...
    def run(self):
        ...
        df['CapBac_clean'] = df['CongViec'].apply(self._extract_rank_strict)
        df['SoLuongTuyen_clean'] = df.apply(self.clean_quantity, axis=1)
        df['HocVan_clean'] = df.apply(self.clean_education, axis=1) 
        # Hình thức & WorkMode
