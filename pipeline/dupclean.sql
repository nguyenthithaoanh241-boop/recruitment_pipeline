...
BEGIN
    ...
    BEGIN TRY
        ...
        ;WITH JsonData AS (
            ...
            WITH (
                ...
                -- Đọc key tiếng Việt từ Python
                YeuCauKiNangCung_clean NVARCHAR(MAX) '$.YeuCauKiNangCung_clean',
                YeuCauKiNangMem_clean NVARCHAR(MAX) '$.YeuCauKiNangMem_clean',
        ...
        WHEN MATCHED THEN
            UPDATE SET
                ...
                -- Map cột
                HardSkills = Source.YeuCauKiNangCung_clean, 
                SoftSkills = Source.YeuCauKiNangMem_clean,
        ...
        WHEN NOT MATCHED BY TARGET THEN
            INSERT (
                ...
                LinhVuc, LinhVuc_clean, HocVan, HocVan_clean,
            ...
            VALUES (
                ...
                Source.PhanLoaiKinhNghiem, 
                
                -- Map cột
                Source.YeuCauKiNangCung_clean, Source.YeuCauKiNangMem_clean,
        ...
        -- [QUAN TRỌNG]: OUTPUT nằm ở đây, không có dấu chấm phẩy ở dòng trên
        ...
        VALUES (
            'sp_Import_FactCleanJobs_JSON', @StartTime, GETDATE(),
            DATEDIFF(MILLISECOND, @StartTime, GETDATE()) / 1000.0,
            'Success', @TotalInputRows, @InsertedCount, @UpdatedCount
        );
    ...
    BEGIN CATCH
        ...
        VALUES (
            'sp_Import_FactCleanJobs_JSON', @StartTime, GETDATE(),
            DATEDIFF(MILLISECOND, @StartTime, GETDATE()) / 1000.0,
            'Failed', @TotalInputRows, @ErrorMessage
        );
        THROW; 
    END CATCH
END
